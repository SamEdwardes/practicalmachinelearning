...
### In sample error
```{r}
predict_in <- predict(model_1, newdata = training_test, type = "class")
table(predict_in)
```

Confusion matrix
```{r}
confusionMatrix(predict_in, training_test$classe)
```

### Estimated out of sample error
Predict using the training data that was set aside
```{r}
predict_out <- predict(model_1, newdata = training_test, type = "class")
table(predict_out)
```
...
First the same transformations that were applied to the training data must be applied to the test data.
```{r}

```

We can now use the predict function generate our results.
```{r}
predict_test
```
